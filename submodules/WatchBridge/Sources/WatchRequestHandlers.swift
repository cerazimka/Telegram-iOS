            ...
            |> mapToSignal({ context -> Signal<Bool, NoError> in
                if let context = context {
                    ...
                    if let args = subscription as? TGBridgeSendTextMessageSubscription {
                        ...
                        messageSignal = .single((.message(text: args.text, attributes: [], inlineStickers: [:], mediaReference: nil, threadId: nil, replyToMessageId: replyMessageId.flatMap { EngineMessageReplySubject(messageId: -e, quote: nil, innerSubject: nil) }, replyToStoryId: nil, localGroupingKey: nil, correlationId: nil, bubbleUpEmojiOrStickersets: []), peerId))
